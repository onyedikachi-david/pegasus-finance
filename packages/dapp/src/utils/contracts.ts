...
export const fetchParamsRaw = async () => {
  try {
    ...
    const debt = await pegasusUSD.totalSupply();
...
export const addCollateral = async (vaultId: number, collAmount: string) => {
  try {
    const tx = await vaultManager.increaseVaultColl(
      vaultId,
      ethers.parseEther(collAmount),
      {
        value: ethers.parseEther(collAmount),
      }
    );
...
export const withdrawCollateral = async (
  ...
  try {
    const tx = await vaultManager.decreaseVaultColl(
      vaultId,
      ethers.parseEther(collAmount)
    );
    await tx.wait();
...
export const mintDebt = async (vaultId: number, debtAmount: string) => {
  try {
    const tx = await vaultManager.increaseVaultDebt(
      vaultId,
      ethers.parseUnits(debtAmount, 18)
    );
    await tx.wait();
  } catch (err: any) {
    throw err;
...
export const returnDebt = async (vaultId: number, debtAmount: string) => {
  try {
    const tx = await vaultManager.decreaseVaultDebt(
      vaultId,
      ethers.parseUnits(debtAmount, 18)
    );
    await tx.wait();
